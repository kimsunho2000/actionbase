...
const Followers: React.FC = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const ownerFolloweesPayload = await scan(
        "social",
        "user_follows",
        "created_at_desc",
        owner.id,
      ...
      const ownerFollowees = ownerFolloweesPayload.edges
        .map(edge => {
          return users.find(u => u.id === edge.source);
        })
      ...
      const followingUsersSet = new Set(ownerFollowees);
  ...
  const {handleFollowingToggle} = useFollowingToggle(
    "doki",
    {
      onSuccess: (isFollowing, followersCount, userId) => {
        // Only update button state, don't refetch data
