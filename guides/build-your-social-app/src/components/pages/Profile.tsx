...
import {postDetails, users} from "../../modules/dummy";
...
const Profile: React.FC = () => {
  ...
  const {handleFollowingToggle} = useFollowingToggle(
    "doki",
    {
      onSuccess: (isFollowing, followersCount, postId) => {
  ...
  const handleFollowingToggleWrapper = async () => {
    await handleFollowingToggle(owner.id, isFollowing);
  };

  useStepAutoAdvance([5, 8, 11, 12, 15], [posts]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const getMyFollowingPayload = await get(
          "social",
          "user_follows",
          "doki",
          owner.id
        )
        if (getMyFollowingPayload.count > 0) {
          setIsFollowing(true)
        }

        const countOfFolloweesPayload = await count(
          "social",
          "user_follows",
          owner.id,
          "IN"
        )
        setFollowers(countOfFolloweesPayload.counts[0].count)

        const countOfFollows = await count(
          "social",
          "user_follows",
          owner.id,
          "OUT"
        )
        setFollowings(countOfFollows.counts[0].count)

        const postsPayload = await scan(
          "social",
          "user_posts",
          "created_at_desc",
          owner.id,
        ...
        <div className="profile-scroll">
          <div className="app">
            ...
            <div className="profile-stats-bottom" id="profile-stats-bottom">
              ...
              <div className="stat-item" onClick={() => navigate("/followers/" + owner.id)}>
                <span className="stat-label">Followers</span>
                <span className="stat-count">{followers}</span>
              </div>
              <div className="stat-item" id="profile-follows" onClick={() => navigate("/followings/" + owner.id)}>
              ...
              <div className="empty-posts">
                <div className="empty-posts-content">
                  ...
                  <div className="empty-posts-text">
                    ...
                    <div className="empty-posts-description">Photos and videos posted by {owner?.name} will be displayed here</div>
                  </div>
                </div>
              </div>
            ) : (
              <div className="posts-grid">
                {posts.map((post, idx) => (
                  <div id={"profile-post-" + idx}
                       key={idx} className="grid-item" style={{background: post.image}} onClick={() => navigate('/post/' + post.id)}>
