          ...
          apiLogs.map((log, index) => {
            ...
            return (
              <React.Fragment key={log.id}>
                ...
                <div
                  className={`api-log-row ${isExpanded ? 'expanded' : ''} ${!log.success ? 'error' : ''}`}
                >
                  <div
                    className={`api-log-row-main ${hasExpandableContent ? 'clickable' : ''}`}
                    onClick={() =>
                      hasExpandableContent && toggleExpandedPayload(log.id, hasRequest)
                    }
                  >
                    ...
                    <div className="api-log-col-type">
                      <span className={`api-log-type api-log-type-${log.apiType.toLowerCase()}`}>
                        {log.apiType}
                      </span>
                    </div>
                    <div className="api-log-col-name">
                      <span
                        className={`api-log-expand-icon ${!hasExpandableContent ? 'hidden' : ''}`}
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
                    ...
                    <div className="api-log-col-method">
                      <span className={`api-log-method api-log-method-${log.method.toLowerCase()}`}>
                        {log.method}
                      </span>
                    </div>
                    ...
                    <div className="api-log-col-latency">
                      <span
                        className={`api-log-latency ${log.latencyMs !== undefined && log.latencyMs < 100 ? 'fast' : ''}`}
                      >
