...
This API provides query operations for edges (unique-edge). For the difference between unique-edge and multi-edge, see [FAQ](/faq/#what-is-the-difference-between-unique-edge-and-multi-edge).

> **Note:** Authentication is not yet implemented. The `Authorization` header is reserved for future use.

For conceptual understanding, see [Query](/design/query/).
