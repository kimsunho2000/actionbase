...
> **Note:** For multi-edge cases (e.g., multiple gifts from the same user to the same recipient), each edge requires a unique `id`. See [FAQ](/faq/#what-is-the-difference-between-unique-edge-and-multi-edge) for details.
